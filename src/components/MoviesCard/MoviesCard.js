...
function MoviesCard(props) {
  ...
  const {country, director, duration, year, description,  trailerLink, nameRU, nameEN, id, onLike, onDislike} = props;
  const image = location.pathname === '/saved-movies'? props.image : `${movieImageBaseUrl}${props.image.url}` ;
  const thumbnail = location.pathname === '/saved-movies'? props.thumbnail :
    `${movieImageBaseUrl}${props.image.formats.thumbnail.url}`;
 ...
 function  displayDuration()  {
  ...
  function handleLike() {
    onLike({country, director, duration, year, description, image, trailerLink, thumbnail, nameRU, nameEN, id})
  }
  ...
  function handleDislike() {
   ...
   return (<div className={'movies-card'}>
      ...
      <p className={'movies-card__duration'}>{
        duration < MINUTESINHOUR? `${displayDuration()}м` : `${displayDuration().hours}ч ${displayDuration().minutes}м`
      }</p>
    </div>)
