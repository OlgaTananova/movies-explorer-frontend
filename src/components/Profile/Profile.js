  ...
  function Button() {
    if (isEditProfile === true) {
      return (
        ...
        profile-form__button_type_submit' type={'submit'}>Сохранить</button>
      )
    } else {
      return(
        <button className='profile-form__button profile-form__button_type_edit'
                type={'button'} onClick={onEditProfile}>Редактировать</button>
      )
    }
  }
  return (
    <div className='profile'>
      <div className='section section_type_profile'>
        <form className='profile-form' name='profile-form' onSubmit={onFormSubmit} noValidate={true}>
          ...
          <span className='profile-form__error profile-form__error_type_api'>Что-то пошло не так</span>
          <Button />
        </form>
      </div>
